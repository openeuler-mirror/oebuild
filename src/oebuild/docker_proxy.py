...
class DockerProxy:
    ...
    def container_exec_command(
        self, container: Container, command, user: str = '', params=None
    ):
        """
        ...
        res = container.exec_run(
            ...
            user=user,
            workdir=None
            if 'work_space' not in params
            else params['work_space'],
            stderr=True,
            stdout=True,
    ...
    def create_container(
        self, image: str, parameters: str, volumes: List, command: str
    ) -> Container:
        """
        ...
        res = subprocess.run(
            run_command, shell=True, capture_output=True, check=True, text=True
        )
        if res.returncode != 0:
    ...
    def check_change_ugid(self, container: Container, container_user):
        ...
        res = self.container_exec_command(
            ...
            params={'work_space': f'/home/{container_user}', 'stream': False},
        )
    ...
    def change_container_uid(
        self, container: Container, uid: int, container_user
    ):
        """
    ...
    def change_container_gid(
        self, container: Container, gid: int, container_user
    ):
        """
