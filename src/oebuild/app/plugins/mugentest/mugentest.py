...
class MugenTest(OebuildCommand):
    ...
    def do_add_parser(self, parser_adder) -> argparse.ArgumentParser:
        ...
        parser.add_argument(
            '--ip', required=True, help='IP address for remote testing'
        )
        parser.add_argument(
            '--user', required=True, help='Username for remote login'
        )
        parser.add_argument(
            '--password', required=True, help='Password for remote login'
        )
        parser.add_argument(
            '--port',
            required=False,
    ...
    def do_run(self, args: argparse.Namespace, unknown=None):
        ...
        if not self.is_mugen_installed(mugen_path):
            print(
                f'Mugen not found at {mugen_path}. Please install Mugen first '
        ...
        if not args.ip or not args.user or not args.password:
            logger.error(
                'For remote testing, --ip, --user, and --password are required.'
            )
            return
    ...
    def select_test_suite(self, mugen_path, args):
        ...
        choice = int(
            input(
                f'Enter the number of the test suite to run (1-{len(test_suites)}): '
            )
        )
    ...
    def run_mugen_test(self, mugen_path, suite, args):
        ...
        try:
            ...
            if result.returncode == 0:
                print(
                    'Successfully configured and connected to the remote environment.'
                )
            else:
                print(
                    f'Failed to configure the remote environment.'
            ...
            if result.returncode == 0:
                ...
                print(
                    f'Test suite {suite} failed with return code {result.returncode}.'
                )
