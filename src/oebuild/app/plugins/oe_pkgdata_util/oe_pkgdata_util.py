...
class OePkgdataUtil(OebuildCommand):

    help_msg = 'execute oe-pkgdata-util command'
    description = textwrap.dedent("""
            The oe-pkgdata-util command queries the pkgdata files written
            out during do_package. It runs non-interactively and all
            arguments are passed through to oe-pkgdata-util directly.
            Example: oebuild oe-pkgdata-util lookup-pkg busybox
            """)
    ...
    def __init__(self):
        ...
        super().__init__(
            'oe-pkgdata-util', self.help_msg, self.description
        )
  ...
  %(prog)s [arguments] [--with-docker-image]
  All arguments will be passed through to oe-pkgdata-util directly.
  --with-docker-image
        ...
        the command example like:
            oebuild oe-pkgdata-util lookup-pkg busybox
            oebuild oe-pkgdata-util list-pkg-files busybox
            oebuild oe-pkgdata-util find-path /usr/bin/ls
""",
        )
    ...
    def do_run(self, args: argparse.ArgumentParser, unknown=None):
        ...
        if len(unknown) == 0:
            logger.error(
                'Please specify arguments for oe-pkgdata-util, '
                'for example: oebuild oe-pkgdata-util lookup-pkg busybox'
            )
            sys.exit(-1)

        command = 'oe-pkgdata-util ' + ' '.join(unknown)
        ...
        if (
            compile_param.no_layer is None
            or compile_param.no_layer is not True
        ):
        ...
        in_container.exec(
            parse_env=parse_env, compile_param=compile_param, command=command
        )
