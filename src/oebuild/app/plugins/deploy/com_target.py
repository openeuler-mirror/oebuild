        ...
        if self.client is not None:
            try:
                ...
                self.client.delete_container(
                    container=container, is_force=True
                )
...
""")
    ...
    def exec(self, str_args: str, fun):
        ...
        if not self._check_if_docker_compile():
            logger.error(
                'The deploy function only be supported in working with docker build'
            )
            sys.exit(-1)
        ...
        if not self._check_conf_directory():
            logger.error(
                'You must work in a exist build directory, '
        ...
        content = oebuild_util.add_bashrc(
            content=content,
            line=(
                f'mv -f /home/{oebuild_const.CONTAINER_USER}/{self.old_bashrc} '
                f'/home/{oebuild_const.CONTAINER_USER}/.bashrc'
            ),
        )
        content = oebuild_util.add_bashrc(
            content=content, line=f'{TARGET_SCRIPT_NAME} {fun} {str_args}'
        )
    ...
    def _check_conf_directory(
        ...
        if not os.path.exists(
            os.path.join(self.work_dir, 'conf/bblayers.conf')
        ):
            return False
        return True
    ...
    def _check_if_docker_compile(self):
        ...
        parse_compile = ParseCompileParam.parse_to_obj(
            compile_param_dict=compile_dict
        )
    ...
    def _make_and_copy_lib(self, container: Container):
        ...
        container.exec_run(
            f'chmod 755 /home/openeuler/{TARGET_DIR_NAME}/{TARGET_SCRIPT_NAME}'
        )
    ...
    def deal_env_container(self, docker_image: str):
        ...
        volumns.append(
            self.configure.source_dir() + ':' + oebuild_const.CONTAINER_SRC
        )
        volumns.append(
            os.path.join(self.configure.build_dir(), cwd_name)
            + ':'
    ...
    def bak_bash(self, container: Container):
        ...
        self.client.container_exec_command(
            container=container,
            command=(
                f'cp /home/{oebuild_const.CONTAINER_USER}/.bashrc '
    ...
    def init_bash(self, container: Container):
        ...
        init_oe_command = (
            f'. {oebuild_const.CONTAINER_SRC}/yocto-poky/oe-init-build-env \
    ...
    def update_bashrc(self, container: Container, content: str):
        ...
        container.exec_run(
            cmd=(
                f'mv /home/{oebuild_const.CONTAINER_USER}/{tmp_file} '
                f'/home/{oebuild_const.CONTAINER_USER}/.bashrc'
            ),
            user='root',
        )
    ...
    def _restore_bashrc_content(self, old_content):
        ...
        for line in old_content.split('\n'):
            line: str = line
            if (
                line.endswith(oebuild_const.BASH_END_FLAG)
                or line.replace(' ', '') == ''
            ):
                continue
    ...
    def _check_change_ugid(self, container: Container):
        ...
        pattern = re.compile(
            r'(?<=uid=)\d{1,}(?=\(' + oebuild_const.CONTAINER_USER + r'\))'
        )
        ...
        if match_uid:
            ...
            raise ValueError(
                f'can not get container {oebuild_const.CONTAINER_USER} uid'
            )
        # get gid from container in default user
        pattern = re.compile(
            r'(?<=gid=)\d{1,}(?=\(' + oebuild_const.CONTAINER_USER + r'\))'
        )
        ...
        if match_gid:
            ...
            raise ValueError(
                f'can not get container {oebuild_const.CONTAINER_USER} gid'
            )
