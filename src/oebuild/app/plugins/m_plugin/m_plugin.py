...
class MPlugin(OebuildCommand):
    ...
    def __init__(self):
        ...
        self.oebuild_plugin_repository = pathlib.Path(
            self.oebuild_plugin_path, 'appends'
        )
...
""",
    ...
    def do_run(self, args: argparse.Namespace, unknown=None):
        ...
        if command == 'install':
            ...
            if args.file and os.path.exists(args.file):
                ...
                file = str(
                    pathlib.Path(args.dir_path, args.major.split('/')[-1])
                )
                ...
                sys.exit(0)
            logger.error(
                'the %s not exist, please check the plugin file path',
                args.file,
            )
            sys.exit(1)
        elif command == 'list':
            ...
            self.enable_disable_plugin(
                command=command, plugin_dict=plugin_dict, args=args
            )
        elif command == 'remove':
    ...
    def list_plugin(self, plugin_dict):
        ...
        if plugin_dict and 'plugins' in plugin_dict:
            ...
            for plugin_data in plugin_dict['plugins']:
                print(
                    f'{str(plugin_data["name"]).ljust(20)}'
            ...
            for plugin_data in plugin_dict['plugins']:
                if plugin_data['name'] == args.plugin_name:
                    ...
                    oebuild_util.write_yaml(
                        self.oebuild_plugin_yaml_path, plugin_dict
                    )
    ...
    def _install_for_dir_check(self, args, file):
        ...
        if not os.path.exists(file):
            logger.error(
                'the %s not exist, please check the plugin file path', file
            )
            sys.exit(-1)
    ...
    def _install_param_check(self, args, plugin_dict):
        ...
        if (
            args.plugin_name in self.command_ext.keys()
    ...
    def create_or_update_plugin_yaml(
        self, plugin_name, class_name, python_file_name, plugin_dict
    ):
        """
        ...
        with open(file_path, 'r', encoding='UTF-8') as file:
            ...
            for file_line in file:
                ...
                if file_line.startswith('class') or file_line.startswith(
                    ...
                    if (
                        re.search(
                            r'((?<=class)\s+\w+\(OebuildCommand\))', file_line
                        )
                        and not class_name
                    ):
                        class_name = (
                            re.search(r'(?<=class)\s+\w+', file_line)
                            .group()
                            .strip()
                        )
            ...
            for plugin_data in plugin_dict['plugins']:
                if plugin_data['name'] == plugin_name:
                    ...
                    subprocess.check_output(
                        f'rm -rf {delete_path}', shell=True
                    )
                    oebuild_util.write_yaml(
                        self.oebuild_plugin_yaml_path, plugin_dict
                    )
    ...
    def install_plugin(
        self, install_plugin_object, plugin_dict, plugin_dict_old
    ):
        """
        ...
        if file_name_dir and not os.path.exists(
            pathlib.Path(file_dir_path, file_name_dir)
        ):
        ...
        if command_info.returncode != 0:
            ...
            oebuild_util.write_yaml(
                self.oebuild_plugin_yaml_path, plugin_dict_old
            )
