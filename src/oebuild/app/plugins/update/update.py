...
""",
    ...
    def get_layer_repo(
        ...
        if os.path.exists(os.path.join(os.getcwd(), 'compile.yaml')):
            compile_param_dict = oebuild_util.read_yaml(
                os.path.join(os.getcwd(), 'compile.yaml')
            )
    ...
    def get_basic_repo(
        ...
        local_dir = os.path.join(
            self.configure.source_dir(), yocto_config.path
        )
        yocto_repo = OGit(
            repo_dir=local_dir,
    ...
    def docker_image_update(self, docker_tag=None):
        ...
        check_docker_tag = CheckDockerTag(
            docker_tag=docker_tag, configure=self.configure
        )
        if docker_tag is not None:
            if (
                check_docker_tag.get_tag() is None
                or check_docker_tag.get_tag() == ''
            ):
                check_docker_tag.list_image_tag()
                return
            docker_image = (
                docker_config.repo_url + ':' + check_docker_tag.get_tag()
            )
        else:
