...
class BitbakeGetvar(OebuildCommand):

    help_msg = 'get bitbake variable value'
    description = textwrap.dedent("""
            The bitbake-getvar command gets the value of a bitbake variable
            in the build environment. It runs non-interactively and requires
            a variable name as argument.
            Example: oebuild bitbake-getvar MACHINE
            """)
    ...
    def __init__(self):
        ...
        super().__init__('bitbake-getvar', self.help_msg, self.description)
  ...
  All arguments will be passed through to bitbake-getvar directly.
  --with-docker-image
        ...
        the command example like:
            oebuild bitbake-getvar MACHINE
            oebuild bitbake-getvar --value MACHINE
            oebuild bitbake-getvar -r busybox PV
""",
        )
    ...
    def do_run(self, args: argparse.ArgumentParser, unknown=None):
        ...
        if len(unknown) == 0:
            logger.error(
                'Please specify arguments for bitbake-getvar, '
                'for example: oebuild bitbake-getvar MACHINE'
            )
            sys.exit(-1)

        command = 'bitbake-getvar ' + ' '.join(unknown)
