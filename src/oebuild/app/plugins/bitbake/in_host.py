...
class InHost(BaseBuild):
    ...
    def exec(self, compile_param: CompileParam, command):
        ...
        self._mk_build_sh(
            nativesdk_dir=compile_param.nativesdk_dir, build_dir=os.getcwd()
        )
        self.init_bitbake()
...
initialization operations""")
        ...
        if command is not None and command != '':
            ...
            append_str = (
                f"sed -i '/{oebuild_const.BASH_END_FLAG}/d' $HOME/.bashrc"
            )
            ...
            new_content = self.bashrc.add_bashrc(
                content, line=source_build_str
            )
    ...
    def _mk_build_sh(self, nativesdk_dir, build_dir):
        ...
        template_dir = (
            f'{self.configure.source_dir()}/yocto-meta-openeuler/.oebuild'
        )
        set_template = f'export TEMPLATECONF="{template_dir}"'
        init_oe_command = (
            f'. {self.configure.source_dir()}/yocto-poky/oe-init-build-env \
            {build_dir}'
        )
