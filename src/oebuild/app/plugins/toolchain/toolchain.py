...
""",
    ...
    def do_run(self, args: argparse.Namespace, unknown=None):
        ...
        if (
            unknown is not None
            and len(unknown) >= 2
            and unknown[0] == 'setlib'
        ):
            self.toolchain_dict['llvm_lib'] = unknown[1]
            oebuild_util.write_yaml(
                self._toolchain_yaml_path, self.toolchain_dict
            )
        ...
        if unknown is None or len(unknown) == 0:
            ...
            build_dir = os.path.join(
                oebuild_const.CONTAINER_BUILD, os.path.basename(os.getcwd())
            )
            docker_exec_list = [
                'docker',
                'exec',
...
you can run oebuild toolchain aarch64 or oebuild toolchain config_aarch64""")
    ...
    def _deal_container(self, parse_env):
        ...
        if not self.client.is_image_exists(
            self.toolchain_obj.docker_param.image
        ):
            print(
                f'the {self.toolchain_obj.docker_param.image} not exists, now pull it'
            )
            self.client.pull_image_with_progress(
                self.toolchain_obj.docker_param.image
            )
    ...
    def _set_llvm_pre(self):
        if self.toolchain_obj.kind != oebuild_const.LLVM_TOOLCHAIN:
            return
        if (
            self.toolchain_obj.llvm_lib is None
            or self.toolchain_obj.llvm_lib == ''
        ):
            logger.error("""
...
pointed it first""")
    ...
    def _add_llvmlib_to_volumn(self):
        ...
        if not check_vol:
            self.toolchain_obj.docker_param.volumns.append(
                self.toolchain_obj.llvm_lib
                + ':'
                + oebuild_const.CONTAINER_LLVM_LIB
            )
    ...
    def _build_gcc(self, config_name: str):
        ...
        self.client.container_exec_command(
            ...
            user=oebuild_const.CONTAINER_USER,
            params={'work_space': build_dir, 'stream': False},
        )
        content = self.bashrc.get_bashrc_content()
    ...
    def _run_upenv(self, kind):
        if kind == oebuild_const.GCC_TOOLCHAIN:
            ...
            src_cross_dir = os.path.join(
                Configure().source_yocto_dir(), '.oebuild/cross-tools'
            )
            subprocess.run(
                f'cp -ru  {src_cross_dir}/* ./', shell=True, check=False
            )
            # replace MANIFEST param in configs/config.xml
            replace_manifest = (
                "sed -i \"`grep -n 'MANIFEST=' configs/config.xml "
            ...
            src_llvm_dir = os.path.join(
                Configure().source_yocto_dir(), '.oebuild/llvm-toolchain'
            )
            subprocess.run(
                f'cp -ru {src_llvm_dir}/* ./', shell=True, check=False
            )
