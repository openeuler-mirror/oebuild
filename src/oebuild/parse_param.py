...
class ParseCompileParam:
    ...
    def parse_to_obj(compile_param_dict) -> CompileParam:
        ...
        if (
            'docker_param' in compile_param_dict
            and compile_param_dict['docker_param'] is not None
        ):
        ...
        return CompileParam(
            build_in=get_value_from_dict(
                'build_in', compile_param_dict, oebuild_const.BUILD_IN_DOCKER
            ),
            machine=get_value_from_dict('machine', compile_param_dict, None),
            toolchain_type=get_value_from_dict(
                'toolchain_type', compile_param_dict, None
            ),
            no_layer=get_value_from_dict('no_layer', compile_param_dict, None),
            toolchain_dir=get_value_from_dict(
                'toolchain_dir', compile_param_dict, None
            ),
            llvm_toolchain_dir=get_value_from_dict(
                'llvm_toolchain_dir', compile_param_dict, None
            ),
            nativesdk_dir=get_value_from_dict(
                'nativesdk_dir', compile_param_dict, None
            ),
            sstate_mirrors=get_value_from_dict(
                'sstate_mirrors', compile_param_dict, None
            ),
            sstate_dir=get_value_from_dict(
                'sstate_dir', compile_param_dict, None
            ),
            tmp_dir=get_value_from_dict('tmp_dir', compile_param_dict, None),
            cache_src_dir=get_value_from_dict(
                'cache_src_dir', compile_param_dict, None
            ),
            repos=None if len(repos) == 0 else repos,
            local_conf=get_value_from_dict(
                'local_conf', compile_param_dict, None
            ),
            layers=get_value_from_dict('layers', compile_param_dict, None),
            docker_param=docker_param,
            bitbake_cmds=get_value_from_dict(
                'bitbake_cmds', compile_param_dict, None
            ),
        )
    ...
    def parse_to_dict(compile_param: CompileParam):
        ...
        if compile_param.llvm_toolchain_dir is not None:
            compile_obj['llvm_toolchain_dir'] = (
                compile_param.llvm_toolchain_dir
            )
...
class ParseToolchainParam:
    ...
    def parse_to_obj(toolchain_param_dict) -> ToolchainParam:
        ...
        return ToolchainParam(
            ...
            gcc_configs=None
            if 'gcc_configs' not in toolchain_param_dict
            else toolchain_param_dict['gcc_configs'],
            llvm_lib=None
            if 'llvm_lib' not in toolchain_param_dict
            else toolchain_param_dict['llvm_lib'],
            docker_param=ParseDockerParam.parse_to_obj(
                toolchain_param_dict['docker_param']
            ),
        )
    ...
    def parse_to_dict(toolchain_param_obj: ToolchainParam):
        ...
        res['docker_param'] = ParseDockerParam.parse_to_dict(
            toolchain_param_obj.docker_param
        )
        return res
