...
def get_nativesdk_environment(
    nativesdk_dir=oebuild_const.NATIVESDK_DIR, container: Container = None
):
    """
    ...
    if container is None:
        ...
        res = container.exec_run(
            'ls -al', user=oebuild_const.CONTAINER_USER, workdir=nativesdk_dir
        )
        if res.exit_code != 0:
            logger.error(
        ...
        for item in list_items:
            ...
            if len(item_split) <= 0:
                continue
            ret = re.match(
                '^(environment-setup-)', item_split[len(item_split) - 1]
            )
...
def get_config_yaml_dir():
    ...
    """
    return os.path.join(
        get_base_oebuild(), 'app/conf', oebuild_const.CONFIG_YAML
    )
...
def get_plugins_yaml_path():
    ...
    """
    return os.path.join(
        get_base_oebuild(), 'app/conf', oebuild_const.PLUGINS_YAML
    )
...
def get_upgrade_yaml_dir():
    ...
    """
    return os.path.join(
        get_base_oebuild(), 'app/conf', oebuild_const.UPGRADE_YAML
    )
...
def init_bashrc_content(old_content, init_command: list):
    ...
    for command in init_command:
        new_content = (
            new_content + command + oebuild_const.BASH_END_FLAG + '\n'
        )
...
def download_repo_from_manifest(repo_list, src_dir, manifest_path):
    ...
    for repo_name in repo_list:
        ...
        if repo_name in manifest:
            print(repo_name)
            repo_obj: RepoParam = ParseRepoParam.parse_to_obj(
                manifest[repo_name]
            )
            repo_git = OGit(
                repo_dir=repo_dir, remote_url=repo_obj.remote_url, branch=None
            )
...
def sync_repo_from_cache(repo_list, src_dir, cache_src_dir):
    ...
    for repo_name in repo_list:
        if os.path.exists(f'{src_dir}/{repo_name}'):
            continue
        subprocess.run(
            f'mkdir -p {src_dir}/{repo_name}', shell=True, check=True
        )
        subprocess.run(
            f'rsync -a {cache_src_dir}/{repo_name}/ {src_dir}/{repo_name}/',
            shell=True,
            check=True,
        )
